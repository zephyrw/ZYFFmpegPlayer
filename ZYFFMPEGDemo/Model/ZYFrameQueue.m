...
typedef NS_ENUM(NSInteger, ZYFFFrameType) {
    ZYFFFrameTypeVideo,
    ZYFFFrameTypeAudio
};

@interface ZYFrameQueue ()
...
// frame pool
@property (assign, nonatomic) ZYFFFrameType type;
...
+ (instancetype)videoQueue {
    return [[self alloc] initWithType:ZYFFFrameTypeVideo capacity:60];
}

+ (instancetype)audioQueue {
    return [[self alloc] initWithType:ZYFFFrameTypeAudio capacity:500];
}

- (instancetype)initWithType:(ZYFFFrameType)type  capacity:(NSInteger)num
{
    if (self = [super init]) {
        self.type = type;
...
- (__kindof ZYFFFrame *)getUnuseFrame {
    ...
    if (self.unuseFrames.count > 0) {
        ...
        if (self.type == ZYFFFrameTypeVideo) {
